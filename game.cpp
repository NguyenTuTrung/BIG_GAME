...
bool game::initGraphic()
{
	...
	else
	{
		...
		gWindow = SDL_CreateWindow("NguyenTuTrung-SDL2", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
...
void game::renderMessageOne()
{
	LTexture image;
	image.Load("./res/image/messageOne.png", 1);
	image.Render(0, 0);
	image.free();
}
...
{
	LTexture image;
	image.Load("./res/image/menu.png", 1);
	image.Render(0, 0);
	image.free();
}
...
void game::replay()
{
	...
	image.Render((SCREEN_WIDTH - image.getWidth()) / 2 + 75, 380);
	image.free();
}
...
{
	LTexture image;
	image.Load("./res/image/continue.png", 1);
	image.Render((SCREEN_WIDTH - image.getWidth()) / 2 - 75, 380);
	image.free();
}
...
bool game::checkReplay()
{
	int x, y;
	SDL_GetMouseState(&x, &y);
	if (x > (SCREEN_WIDTH - 100) / 2 + 75 && x < (SCREEN_WIDTH + 100) / 2 + 75 && y > 380 && y < 380 + 60)
	{
		return true;
	}
	return false;
